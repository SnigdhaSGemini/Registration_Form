...
regi.addEventListener("click",(e)=>{
    ...
    var check= firstn.value.match(vfirst); if(check==null){
        DisplayError(firstn,"Invalid First Name");
        /*alert("Invalid First Name");*/}
    else{
        const vlast =/^[A-Za-z\s]+$/;
    var check=lastn.value.match(vlast); if(check==null){DisplayError(lastn,"Invalid Last Name");
    /*alert("Invalid Last Name");*/}
else{
   const vemail =/^[\w.-]+@[a-zA-Z_-]+?\.[a-zA-Z]{2,3}$/;
    var check= email.value.match(vemail); if(check==null){DisplayError(email,"Invalid Email");
/*alert("Invalid Email");*/}
 else{
    ...
    if (num==null && no.value) {
        ...
        if(p1.value!==p2.value){DisplayError(p1,"Password Mismatched");
